...
export const DeadlinesTimeline: React.FC<DeadlinesTimelineProps> = ({
  ...
  const handleDownloadIcs = () => {
    const url = apiClient.getCalendarDownloadUrl(documentId);
              ...
              <div className="glass-card-interactive rounded-xl p-3.5 border border-slate-800/80 hover:border-slate-700">
                <div className="flex items-start justify-between gap-2">
                  <div className="flex-1">
                    ...
                    {d.description && (
                      <p className="text-xs text-slate-300 leading-relaxed mt-1">{d.description}</p>
                    )}
                  </div>

                  {/* Due Date Badge */}
                  <div className="flex flex-col items-end flex-shrink-0">
                    <span className="px-2.5 py-1 rounded-lg text-xs font-semibold bg-emerald-500/15 text-emerald-300 border border-emerald-500/30 flex items-center gap-1 font-mono">
                    ...
                    {d.page_number && (
                      <button
                        ...
                        className="mt-1.5 px-2 py-0.5 rounded text-[10px] font-medium bg-brand-500/10 hover:bg-brand-500/20 text-brand-400 border border-brand-500/20 flex items-center gap-1 transition-colors"
